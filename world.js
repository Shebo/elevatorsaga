...
var createWorldCreator = function() {
    ...
    creator.createElevators = function(elevatorCount, floorCount, floorHeight, elevatorCapacities) {
        ...
        var elevators = _.map(_.range(elevatorCount), function(e, i) {
            var elevator = new Elevator(2.6, floorCount, floorHeight, elevatorCapacities[i%elevatorCapacities.length], 49);
    ...
    creator.spawnUserRandomly = function(floorCount, floorHeight, floors) {
        ...
        if(currentFloor === 0) {
            ...
            if(_.random(10) === 0 && currentFloor != floorCount-1 && currentFloor != 49) {

                if (_.random(1) === 0){
                    // going up
                    destinationFloor = currentFloor + _.random(1, floorCount-1-currentFloor);
                }else{
                    // going down
